...
class FareTransfer extends Contract {
    ...
    async EnrollCustomer(ctx, id, firstName, lastName, primaryTransit){
        const isExistingCustomer = await this.CustomerExists(ctx, id);
        ...
        await ctx.stub.putState(id, Buffer.from(stringify(sortKeysRecursive(enrollCustomerData))));
        return JSON.stringify(enrollCustomerData);
    }

    // GetCustomer returns the customer info stored in the world state with given id.
    async GetCustomer(ctx, id) {

        const customerData = await ctx.stub.getState(id);
    ...
    async GetCustomerLastFare(ctx, lastTxnId) {
        const lastFareData = await ctx.stub.getState(lastTxnId);
        if(!lastFareData && lastFareData.length === 0){
            throw new Error(`Transit Id ${lastTxnId} does not exist`);
        }

        return lastFareData.toString();
    }

    // UpdatePrimaryTransit updates an existing customer primaryId.
    async UpdatePrimaryTransit(ctx, id, newTransitId) {
        const isValidTransit = utils.isValidTransit(newTransitId);
        if(!isValidTransit){
    ...
    async CustomerExists(ctx, id) {
        const customerData = await ctx.stub.getState(id);
        const isCustomerExists = customerData && customerData.length > 0;
